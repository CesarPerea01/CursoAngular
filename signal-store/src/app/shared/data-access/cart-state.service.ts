...
export class CartStateService {
    ...
    loadProducts$ = this._storageService.loadProducts().pipe(map(products => ({products, loaded: true})))
    ...
    state = signalSlice({
        ...
        selectors:(state)=>({
            ...
            price: ()=> {return state().products.reduce((acc,product) => acc + product.quantity * product.product.price,0)},
        }),
        actionSources: {
